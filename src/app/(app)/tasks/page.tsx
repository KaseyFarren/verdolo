...
export default async function TasksPage() {
  ...
  const [{ data: clients }, { data: tasks }, { data: recurring }, { data: defaults }, { data: members }, { data: budgets }, { data: projects }, { data: phases }] = await Promise.all([
    ...
    supabase.from('projects').select('id, name, client_id').eq('org_id', orgId).neq('status', 'archived').order('name').limit(2000),
    supabase.from('project_phases').select('id, project_id, name').eq('org_id', orgId).order('sort_order').limit(4000),
  ])
