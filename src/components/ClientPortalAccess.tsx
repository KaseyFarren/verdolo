...
type ClientUser = { id: string; invited_email: string | null; status: string; created_at: string }
...
export default function ClientPortalAccess({ supabase, clientId }: { supabase: SupabaseClient; clientId: string }) {
  ...
  async function invite() {
    ...
    const res = await fetch('/api/invite/client', {
      ...
      body: JSON.stringify({ email: trimmed, clientId }),
    })
    ...
    setContacts((prev) => [{ id: crypto.randomUUID(), invited_email: trimmed, status: 'invited', created_at: new Date().toISOString() }, ...prev])
    setEmail('')
